...
const NewAppointment = () => {
  ...
  const consultingReasons = [
    "Accidents", "Acute Back Pain", "Acute Bronchitis", "Acute Contact Dermatitis",
    "Acute migraine / headache", "Acute Eczema Flare-ups", "Acute Kidney Injury",
    "Acute viral fever", "Acute Pelvic Inflammatory Disease (PID)", "Acute Sinusitis",
    "Acute Urticaria", "Alzheimer's Disease", "Allergic cough", "Allergic skin rashes",
    "Ankylosing Spondylitis", "Asthma", "Atrial Fibrillation", "Bipolar Disorder",
    "Boils, abscess", "Breast Cancer", "Chronic Bronchitis", "Chronic Hepatitis (B and C)",
    "Chronic Kidney Disease", "Chronic Migraine", "Chronic Obstructive Pulmonary Disease",
    "Colorectal Cancer", "Common Cold", "Coronary Artery Disease", "COVID-19",
    "Crohn's Disease", "Croup", "Dengue Fever", "Diabetes (Type 1 and Type 2)",
    "Diabetic Nephropathy", "Epilepsy", "Fibromyalgia", "Gastroenteritis",
    "Generalized Anxiety Disorder", "Glomerulonephritis", "Heart Failure", "Head injury",
    "Hypertension (High Blood Pressure)", "Hyperthyroidism", "Hypothyroidism",
    "Injury, cuts, burns, bruise, blow", "Impetigo", "Influenza (Flu)",
    "Irritable Bowel Syndrome (IBS)", "Leukemia", "Lung Cancer", "Major Depressive Disorder",
    "Malaria", "Metabolic Syndrome", "Multiple Sclerosis", "Nephrolithiasis (Kidney Stones)",
    "Non-Alcoholic Fatty Liver Disease", "Osteoarthritis", "Osteoporosis", "Oral Ulcers",
    "Parkinson's Disease", "Peripheral Artery Disease", "Polycystic Kidney Disease",
    "Polycystic Ovary Syndrome (PCOS)", "Post-Traumatic Stress Disorder (PTSD)",
    "Prostate Cancer", "Psoriasis", "Pulmonary Hypertension", "Rheumatoid Arthritis",
    "Schizophrenia", "Scleroderma", "Sjogren's Syndrome", "Sprains and Strains",
    "Strep Throat", "Systemic Lupus Erythematosus (SLE)", "Tooth Pain", "Trauma",
    "Ulcerative Colitis", "Urinary Tract Infection (UTI)", "Other",
  ];
  ...
  useEffect(() => {
    const fetchSlots = async () => {
      if (startDate) {
        ...
        try {
          const res = await axios.post(`${API_URL}/api/patient/checkSlots`,
            { appointmentDate },
            { headers: { Authorization: `Bearer ${token}` } }
          );
  ...
  const validateForm = () => {
    ...
    if (!consultingFor) errors.consultingFor = "Please select consulting person";
    if (!consultingReason) errors.consultingReason = "Please select consulting reason";
  ...
  return (
    <div className="max-w-4xl mx-auto py-10 grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div className="bg-white p-6 rounded-lg shadow space-y-6">
        <h2 className="text-xl font-bold text-center bg-blue-100 p-3 rounded">Book an Appointment</h2>
        ...
        />
        {formErrors.consultingFor && <p className="text-red-500 text-sm">{formErrors.consultingFor}</p>}
        ...
        />
        {formErrors.consultingReason && <p className="text-red-500 text-sm">{formErrors.consultingReason}</p>}
          ...
          {timeSlots.map(time => (
            <button
              ...
              className={`p-2 rounded border transition ${
                ...
                availableSlots.includes(time) ? "bg-white hover:bg-blue-100" :
