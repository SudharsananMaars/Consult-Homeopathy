...
const Messenger = ({ toggleMessenger, isVisible }) => {
  ...
  return (
    <div
      className={`fixed top-0 right-0 h-full w-1/4 bg-white shadow-lg rounded-lg border-2 border-purple-100 z-50 transform transition-transform duration-300 ${
        isVisible ? "translate-x-0" : "translate-x-full"
      }`}
    >
      <div className="p-4 bg-blue-300 text-white flex justify-between ">
        <h3 className="text-lg font-bold text-gray-900">Messenger</h3>
        <button onClick={toggleMessenger} className="text-gray-900">
          <FaReply />
        </button>
      </div>
      ...
      {selectedMessage && (
        <div className="p-4 border-t">
          ...
          <button className="w-full mt-2 p-2 bg-blue-100 text-blue-300 hover:bg-blue-200">
