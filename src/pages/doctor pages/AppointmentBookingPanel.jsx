...
const SlotTypeRow = ({ slot, isEditing, onSave, onEdit, onUpdateField, onUpdateDays, onToggleBooking }) => {
    ...
    return (
        <div className="grid grid-cols-12 items-center gap-4 p-4">
            ...
            <div className="col-span-2">
                {isEditing ? (
                    <PriceInput isEditing={isEditing} value={slot.price} onChange={(val) => onUpdateField(slot.id, 'price', val)} />
                ) : (
                    <div className="border border-gray-200  text-gray-700 text-sm font-semibold rounded-lg block w-full pl-10 pr-3 py-2 relative h-[42px] flex items-center">
                        <IndianRupee className="absolute top-1/2 left-3 transform -translate-y-1/2 text-gray-400" size={16} />
                        {slot.price}
            ...
            <div className="col-span-1 flex justify-center gap-2">
                {isEditing ? (
                    <button onClick={() => onSave(null)} className="text-gray-500 hover:text-green-600 transition-colors">
                        <Check size={16} />
                    </button>
                ) : (
...
const AppointmentBookingPanel = () => {
  ...
  const [slotTypes, setSlotTypes] = useState([
    { id: 1, type: 'Normal', start: '05:00', end: '22:00', price: '500', allowBooking: true, days: [] },
    { id: 2, type: 'Post Working Hours', start: '17:00', end: '22:00', price: '700', allowBooking: true, days: [] },
    { id: 3, type: 'Weekoff', start: '05:00', end: '22:00', price: '600', allowBooking: true, days: ['Sat', 'Sun'] },
    { id: 4, type: 'Opinion Consultation', start: '05:00', end: '22:00', price: '400', allowBooking: true, days: [] },
  ]);
            ...
            {clinicHours.map((hour, index) => (
              <div key={hour.day} className="grid grid-cols-12 items-center gap-4 p-4">
                ...
                <div className="col-span-4 flex items-center gap-2">
                  <CustomTimePicker value={hour.from} onChange={(val) => updateClinicTime(index, 'from', val)} />
                  <span className="text-gray-500 px-2 font-semibold text-[14px]">-</span>
                  <CustomTimePicker value={hour.to} onChange={(val) => updateClinicTime(index, 'to', val)} />
                </div>
