...
const PatientCommunicationCard = ({ filter = "month" }) => {
  ...
  useEffect(() => {
    const fetchMessengerData = async () => {
      ...
      try {
        const response = await fetch(
          `${API_URL}/api/analytics/messenger-summary`,
          {
            method: "PATCH",
            headers: {
              "Content-Type": "application/json",
  ...
  useEffect(() => {
    const fetchQualityData = async () => {
      ...
      try {
        const response = await fetch(
          `${API_URL}/api/analytics/overall-summary`,
          {
            method: "PATCH",
            headers: {
              "Content-Type": "application/json",
  ...
  const renderQualityAssuranceContent = () => {
    ...
    const prescriptionAccuracy =
      summary.prescriptionRevisionRatePercentage !== undefined &&
      summary.prescriptionRevisionRatePercentage !== null
        ? (100 - summary.prescriptionRevisionRatePercentage).toFixed(0)
        : "N/A";

    const averageResponses =
      summary.averageCallsPerPatient !== undefined &&
      summary.averageCallsPerPatient !== null
        ? summary.averageCallsPerPatient.toFixed(1)
        : "N/A";

    const communicationScore =
      summary.averageCommunicationScore !== undefined &&
      summary.averageCommunicationScore !== null
              ...
              <>
                <table className="w-full">
                  <thead className="bg-indigo-500 text-white sticky top-0">
                    <tr>
                      <th className="py-3 px-4 text-left text-sm font-medium w-16">
                        #
                      </th>
                      <th className="py-3 px-4 text-left text-sm font-medium w-48">
                        Category
                      </th>
                      <th className="py-3 px-4 text-left text-sm font-medium flex-1">
                        Feedback Questions
                      </th>
                      <th className="py-3 px-4 text-center text-sm font-medium w-32">
                        Actions
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    {questions.length === 0 ? (
                      <tr>
                        ...
                        >
                          No questions available. Click "Add New Questions" to
                          create one.
                        </td>
                      </tr>
                    ) : (
      ...
      <div className="flex items-start justify-between gap-6 h-full">
        ...
        <div className="flex flex-col gap-2 flex-1">
          <div className="flex items-center justify-between">
            <div className="text-sm font-semibold text-gray-800">Feedback</div>
            <button
