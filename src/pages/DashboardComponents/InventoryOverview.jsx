...
function InventoryOverview() {
    ...
    const fetchOrderData = async () => {
        ...
        try {
            const response = await fetch(`${API_URL}/api/analytics/order-frequency-chart`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
    ...
    const renderVendorChart = () => {
        if (vendorLoading) return <div className="text-xs text-gray-400">Loading...</div>;
        if (!vendorData?.data) return <div className="text-xs text-gray-400">No data</div>;
                ...
                {vendorData.data.map((vendor, idx) => {
                    ...
                    return (
                        <div key={idx} className="flex flex-col items-center flex-1">
                            ...
                            <div className="text-xs text-gray-600 mt-1">{vendor.vendorName}</div>
                        </div>
                    );
                })}
            </div>
    ...
    const renderOrderChart = () => {
        if (orderLoading) return <div className="text-xs text-gray-400">Loading...</div>;
        if (!orderData?.data) return <div className="text-xs text-gray-400">No data</div>;

        const maxValue = Math.max(...orderData.data.map(d => d.y), 500);
        const points = orderData.data.map((point, idx) => {
        ...
        <div className="h-full flex flex-col">
            <div className="flex items-start justify-between gap-4 h-full">
                ...
                <div className="flex flex-col gap-2" style={{ minWidth: '160px' }}>
                    <div>
                        <div className="text-base font-bold text-gray-800">Inventory</div>
                        <div className="text-xs text-gray-500">Threshold Range : <span className="font-semibold text-gray-700">80%</span></div>
                    </div>
                    
                    {/* Status Cards - Compact */}
                    <div className="flex flex-col gap-2">
                        {/* Above Threshold */}
                        <div className="relative bg-white rounded-lg shadow-sm border border-gray-100 pl-3 pr-3 py-2">
                            <div className="absolute left-0 top-0 h-full w-1 rounded-l-lg bg-sky-400" />
                            <div className="text-xs text-gray-600">Above Threshold</div>
                            <div className="text-2xl font-bold text-sky-400 leading-tight">
                        ...
                        <div className="relative bg-white rounded-lg shadow-sm border border-gray-100 pl-3 pr-3 py-2">
                            ...
                            <div className="text-2xl font-bold text-amber-500 leading-tight">
                ...
                <div className="flex-1 flex flex-col">
                    <div className="text-xs font-semibold text-gray-700 mb-1">Vendor Performance</div>
                    <div className="bg-gray-50 rounded-lg p-2 flex-1 flex flex-col">
                        {/* Y-axis labels */}
                ...
                {/* Order Frequency Chart */}
                <div className="flex-1 flex flex-col">
                    <div className="text-xs font-semibold text-gray-700 mb-1">Order Frequency Chart</div>
                    <div className="bg-gray-50 rounded-lg p-2 flex-1 flex flex-col">
                        {/* Y-axis label */}
                ...
                <div className="flex flex-col gap-3" style={{ minWidth: '140px' }}>
                    <div>
                        <div className="text-xs font-semibold text-gray-700 mb-1">Raw Materials</div>
                        <div className="flex items-center gap-2">
                            <div className="flex">
                                {[...Array(6)].map((_, i) => (
                                    <div key={i} className="w-2.5 h-2.5 rounded bg-blue-600 mr-0.5" />
                                ))}
                                {[...Array(4)].map((_, i) => (
                                    <div key={i} className="w-2.5 h-2.5 rounded bg-blue-200 mr-0.5" />
                                ))}
                            </div>
                    ...
                    <div>
                        <div className="text-xs font-semibold text-gray-700 mb-1">Operational</div>
                        <div className="flex items-center gap-2">
                            <div className="flex">
                                {[...Array(3)].map((_, i) => (
                                    <div key={i} className="w-2.5 h-2.5 rounded bg-pink-500 mr-0.5" />
                                ))}
                                {[...Array(2)].map((_, i) => (
                                    <div key={i} className="w-2.5 h-2.5 rounded bg-pink-200 mr-0.5" />
                                ))}
                            </div>
                    ...
                    <div className="mt-2">
                        <div className="text-xs font-semibold text-gray-700 mb-1">Expiry Tracking (Raw)</div>
                        <div>
